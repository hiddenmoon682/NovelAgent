...
- **窗口恢复几何后跑到屏幕外（`setGeometry: ... +329-65091 → +329-32768`，页面看不见）**：`MainWindow.restoreGeometry()` 在 `Component.onCompleted`（窗口映射前）执行，此时 `window.screen`/`availableGeometry` 可能尚未指向真实屏（换屏/摘除副屏/最大化残留），按错误的屏信息做尺寸/位置夹取，可能把窗口铺到屏幕外。
  ...
  - 拖拽钳制：标题栏拖拽 `onPositionChanged` 里加钳制，把窗口**完整留在屏幕内**（四边含下缘都拖不出屏）。基准用**整屏几何 `scr.geometry`（含任务栏区）而非可用区域**，否则窗口拖到任务栏上沿就停、往下拖不动——这样给下方拖拽留足余量仍不出屏；屏引用加主屏回退防拖拽时 `window.screen` 取不到。之前 80px 余量式钳制会让人往下拖时窗体能贴到屏幕下缘以下，已收紧为"拖到底就贴住物理屏下缘"。
...
### 修复 — GUI
- **AI/用户单行消息气泡内文本偏上、下方空隙明显大于上方**：`bubbleText` 此前设了 `lineHeight: root.isUser ? 20 : 1.4`（FixedHeight 20 / Proportional 1.4）。Noto Serif SC 15px 的**自然行高本身约 22px**（对 CJK 已含充足行距），再被人工放大后单行行框被撑到 ~31px，而字形（ink）只有 ~13px 且被 QML 画在行框**顶部**——行框下方多出大片空白，文本看起来严重偏上。实测（像素级）：气泡高 39px、字形高 13px，字形上间隙 8.7px、**下间隙 17.3px（约为上间隙 2 倍）**。
  - 修法：**改用字体自然行高**（删除 `lineHeight`/`lineHeightMode` 两行），单行行框回落到 ~22px。修复后实测：气泡高 30px、字形上间隙 8.7px / **下间隙 8px（上下对称居中）**。多行用自然行距（约 1.47×），阅读正常，气泡形态从 39px 收窄到 30px 更贴合文本。
  - 说明：用户消息与 AI 消息共用同一 `bubbleText`，改前分别为 FixedHeight 20 / Proportional 1.4 两种不同方法（观感不一致、偏的方向相反），改后**统一为字体自然行高**。
- **思考过程（reasoning）展开框小字同样"偏上"**：`reasoningText` 此前 `lineHeight: 1.5`（`fontUi` 12px），同理放大行框使小字偏上。已按同一思路改为**字体自然行高**，与消息气泡统一；渲染验证文字不挤、上下留白对称。
- 说明：此前多次改动（气泡最小宽、尾部换行清洗、reasoning 尾部换行）都只是缓解/处理了"空行撑高"这一关联表象；**单行文本不居中的真正根因是这条人工放大行高**。本次一并给出。

## [2026-08-26] 对话区：移除发言方名字标签 + 气泡贴合文本 + 修正 AI 气泡"文本偏上、底部空隙大"

### 变更 — GUI
- **去掉每条消息旁的名字标签「你 / 墨染」**：`ChatBubble.qml` 里用于标注发言方（用户「你」、AI「墨染」）的头部 `Label` 不需要了，直接删除；同时清理了为此保留的 `showHeader` 属性与 `AgentPanel.qml` 里的 `showHeader: delegateRoot.newTurn` 注入（`newTurn` 仍用于加大不同发言方回合之间的段间距，视觉上仍能区分回合）。空状态居中大字标题「墨染」按用户确认保留。

### 修复 — GUI
- **短消息气泡过宽、右侧空出太多死白（如「你好」）**：气泡宽度按 `Math.max(40, textMeasurer.contentWidth)` 计算，40 是文本区的最小硬下限；对于「你好」这类只有 ~30px 的短文本，文本区被强撑到 40px、气泡被拉到约 64px，实际文字只占左侧、右侧多出一段空白。改用 `Math.max(Theme.gapMd * 2, …)`（24px 保底）作地板，使气泡贴合文本自然宽度（「你好」→约 54px），两侧留白对称、不再右侧空置。
- **AI 长回复气泡内文字"偏上、下方空隙大于上方"**：`formattedText` 的助手路径此前只做 `mdWithHardBreaks`，从不清理**尾部换行/空行**；当模型输出以 `\n`/空行结尾时，`contentHeight` 会把末行下方的空行也计入，气泡底部凭空多出一段空白，视觉上文本偏上、下空隙明显大于上空隙。修法与用户消息 `displayText` 一致：助手路径也 `replace(/\n+$/, "")` 去掉尾部空行，使气泡高度只包裹可见文本，上下留白对称（`Text` 四周本就是对称 padding：上 4/下 4、左 12/右 12）。
- **思考过程（reasoning）展开框同样"文本偏上、下方空隙大"**：根因与消息气泡一致——`reasoningText` 此前直接 `text: root.reasoning`，从不清洗尾部换行；LLM 的 reasoning 常以 `\n` 收尾，`reasoningText.implicitHeight` 就把末行下方空行也算进框高，使框被撑高、可见文本偏上。新增 `displayReasoning`（`reasoning.replace(/\n+$/, "")`）并用于文本，使思考过程框只包裹可见文本、上下留白对称（上 8/下 8）。

## [2026-08-26] 调试页开关飞出框外（布局侧补丁）

### 修复 — GUI
- **开关仍被推到行右缘外**：`ThemedSwitch` 改纯自绘后开关本身不再溢出，但调试行标签用了 `Layout.fillWidth: true`，标签取满宽度后把开关推到 ContentArea 右缘，且行宽受内容区/其它控件宽度计算影响，开关最终被挤出设置弹窗（截图中开关在内容区框外）。修法：去掉标签的 `Layout.fillWidth`，改用 `RowLayout spacing`，让开关紧贴标签排布（「启用调试日志 [开关]」左对齐），位置由布局确定、不再被其它控件推到框外。
