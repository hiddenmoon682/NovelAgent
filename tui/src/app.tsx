...
export const App: React.FC<Props> = ({ api, projectPath }) => {
  ...
  const messagesEnd = useRef<number>(0);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setMessages([{ role: "error", content: `无法连接后端: ${e.message}\n请先启动: novelagent backend -p ./项目` }]);
      }
    })();
    return () => {
      if (sessionId) api.destroySession(sessionId);
    };
  }, []);
  ...
  const sendMessage = () => {
    ...
    const msgIndex = messages.length + 1; // 新 assistant 消息的索引
      ...
      (event: ServerEvent) => {
        switch (event.type) {
          case "content":
            setMessages((prev) => {
              ...
              if (updated[msgIndex]) {
                updated[msgIndex] = {
                  ...updated[msgIndex],
                  content: updated[msgIndex].content + event.delta,
                };
              }
              return updated;
            });
          ...
          case "tool_call":
            setMessages((prev) => [
              ...prev,
              { role: "tool", content: `🔧 ${event.name}`, toolName: event.name },
            ]);
            break;
          case "tool_result":
            setMessages((prev) => [
              ...
              {
                role: "tool",
                content: `  ${event.summary ? event.summary.substring(0, 100) : "完成"}`,
              },
            ]);
            break;
          case "done":
  ...
  return (
    <Box flexDirection="column" height="100%">
      ...
      {/* 底部提示 */}
      <Box>
        <Text dimColor>Ctrl+C 退出 | 输入消息开始写作</Text>
      </Box>
    </Box>
  );
};
