...
//
// 调用方（Agent / QmlBridge）通过这些回调获得逐 token 的流式输出。
...
//   on_tool_start     — 单个工具执行前触发，携带工具名与参数 JSON（由 CoreLoop 触发）
//   on_tool_finish    — 单个工具执行后触发，携带工具名、成功标志与结果内容（由 CoreLoop 触发）
...
struct StreamCallbacks {
    ...
    // 工具执行生命周期（由 CoreLoop 在执行工具前后触发；UI 可据此展示状态卡片。
    // on_tool_start 携带工具名与参数 JSON 原文；on_tool_finish 携带结果内容原文
    // （失败/异常时为空串），显示层可据此展开参数/结果详情。）
    std::function<void(const std::string& tool_name, const std::string& arguments)> on_tool_start;
    std::function<void(const std::string& tool_name, bool ok, const std::string& result)> on_tool_finish;
