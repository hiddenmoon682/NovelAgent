...
public:
    ...
    // ================================================================
    // 渐进式加载查询（供 ProgressiveToolProvider 使用）
    // ================================================================
    ...
    // 按名称获取单个工具定义（未找到返回 nullopt）。
    std::optional<llm::ToolDefinition> getToolDefinition(const std::string& name) const;

    // 按关键词搜索工具（名称或描述包含 query），返回匹配的定义列表。
    std::vector<llm::ToolDefinition> searchTools(const std::string& query) const;

    // 获取所有工具的摘要信息（名称+描述+类别）。
    std::vector<ToolSummary> getToolSummaries() const;
