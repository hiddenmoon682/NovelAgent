#include "agent/tool/ContextualToolProvider.h"
...
ContextualToolProvider::ContextualToolProvider(IToolProvider& tools)
    : tools_(tools) {}
...
ContextualToolProvider::getActiveDefinitions() const {
    ...
    for (auto cat : active_categories_) {
        auto names = tools_.toolNamesByCategory(cat);
        allowed.insert(names.begin(), names.end());
    }

    auto all = tools_.getDefinitions();
