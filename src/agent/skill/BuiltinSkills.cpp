...
- 回复正文与思考过程**一律禁止使用 emoji、表情符号或图形装饰符号**：😀 ✨ 🎉 🔥 ✅ ⚠ 1️⃣ 👉 🔴 等一律不要出现。
- 禁止用符号做装饰（如 `~~`、`==`、`✧`、`➤` 等划线/花边）；表达重点用 `**加粗**` 或分点说明。
- 编号统一用纯文本：`1.` `2.` `3.`；列表用 `-` 或 `*`。

## 正文格式规范（write_chapter / append_to_chapter 等写作工具必须遵守）

- 章节正文一律纯文本：禁止任何 Markdown 标记（`#` 标题、`**` 加粗、`-`/`1.` 列表、`>` 引用、行内代码等），小说正文按自然段落书写。
- 段落之间用空行分隔，直接书写段落文字，不要加装饰符号。
- 章节标题、叙事简报等元数据通过章节工具的字段管理（create_chapter / update_chapter 的 title、synopsis 等），不要写进正文字符串。
