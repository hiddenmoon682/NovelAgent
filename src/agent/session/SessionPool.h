...
private:
    ...
    // 共享线程池。必须声明在 in_flight_ 之后：C++ 成员按声明逆序析构，析构本类时
    // pool_exec_ 先于 in_flight_/in_flight_mutex_ 销毁并 join 全部工作线程；若先销毁
    // 互斥锁，关闭时 join 阻塞期间池任务尾部 in_flight_.erase 会对已析构互斥锁加锁
    // （SessionPool.cpp 任务收尾），构成确定性析构序 UAF。
